...
/**
 * Data access object to insert, find and load {@link Photo}s.
 * 
...
public class PhotoFileSystemDao  {
     ...
     * 
     * @see org.murygin.archive.dao.IDocumentDao#insert(org.Photo.archive.service.Document)
     */
    public void insert(Photo document) {
        try {
     ...
     */
    public List<PhotoMetadata> findByPersonNameDate(String personName, Date date) {
        try {
    ...
    private List<PhotoMetadata> findInFileSystem(String personName, Date date) throws IOException  {
        List<String> uuidList = getUuidList();
        List<PhotoMetadata> metadataList = new ArrayList<PhotoMetadata>(uuidList.size());
        for (String uuid : uuidList) {
            PhotoMetadata metadata = loadMetadataFromFileSystem(uuid);         
    ...
    private boolean isMatched(PhotoMetadata metadata, String personName, Date date) {
        if(metadata==null) {
            return false;
        }
        boolean match = true;
    ...
    private PhotoMetadata loadMetadataFromFileSystem(String uuid) throws IOException {
        PhotoMetadata document = null;
        String dirPath = getDirectoryPath(uuid);
        File file = new File(dirPath);
        System.out.println("File path: "+file.getAbsolutePath());
        if(file.exists()) {
            Properties properties = readProperties(uuid);
            document = new PhotoMetadata(properties);
            
        } 
        return document;
    }
    
    private Photo loadFromFileSystem(String uuid) throws IOException {
       PhotoMetadata metadata = loadMetadataFromFileSystem(uuid);
       if(metadata==null) {
           return null;
       }
       Path path = Paths.get(getFilePath(metadata));
       Photo document = new Photo(metadata);
       document.setFileData(Files.readAllBytes(path));
       return document;
    }

    private String getFilePath(PhotoMetadata metadata) {
    ...
    private void saveFileData(Photo document) throws IOException {
    ...
    public void saveMetaData(Photo document) throws IOException {
            String path = getDirectoryPath(document);
    ...
    private String createDirectory(Photo document) {
        String path = getDirectoryPath(document);
        createDirectory(path);
        return path;
    }

    private String getDirectoryPath(Photo document) {
    ...
    private String getDirectoryPath(String uuid) {
        ...
        System.out.println("Load from "+sb.toString());
